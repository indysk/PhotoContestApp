  ...
  def content_tag_options_for_contest_badge(contest, current_class)
    if contest.is_after_period_voting?
      ...
      if contest.is_already_voted?(current_user)
        {tag: 'div', content: '投票済み', options: {class: "#{current_class} vote_finish"}, value: 3.5 }
      else
      ...
      if contest.is_already_submitted?(current_user)
        {tag: 'a', content: '作品提出済み', options: {href: edit_contest_photo_path(contest, Photo.find_by(contest: contest, user: current_user)), class: "#{current_class} vote_finish"}, value: 2.5 }
      else
        {tag: 'a', content: '作品募集中', options: {href: new_contest_photo_path(contest), class: "#{current_class} vote_finish"}, value: 2 }
      end
    else
      {tag: 'div', content: '準備中', options: {class: "#{current_class} vote_finish"}, value: 1 }
    end
  end
end
