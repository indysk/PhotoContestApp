module ContestsHelper
  def content_tag_options_for_contest_badge(contest, current_class)
    if contest.is_after_period_voting?
      if contest.visible_range_result.to_s == '1' && contest.visible_range_show.to_s == '1'
        {tag: 'div', content: '非公開', options: {class: "#{current_class} show_result_invisible"}, value: 4.7 }
      ...
      if contest.is_already_voted?(current_user)
        {tag: 'div', content: '投票済み', options: {class: "#{current_class} already_voted"}, value: 3.7 }
      elsif contest.visible_range_vote.to_s == '1'
        {tag: 'div', content: '非公開', options: {class: "#{current_class} vote_invisible"}, value: 3.5 }
      else
      ...
      if contest.visible_range_vote.to_s == '1'
        {tag: 'div', content: '非公開', options: {class: "#{current_class} vote_invisible"}, value: 2.5 }
      else
      ...
      if contest.is_already_submitted?(current_user)
        ...
        {tag: 'div', content: '非公開', options: {class: "#{current_class} entry_invisible"}, value: 1.5 }
      else
      ...
      if contest.visible_range_entry.to_s == '1'
        {tag: 'div', content: '非公開', options: {class: "#{current_class} entry_invisible"}, value: 0.5 }
      else
        {tag: 'div', content: '準備中', options: {class: "#{current_class} entry_preparing"}, value: 0 }
      end
    end
  end
  ...
  def content_tag_options_for_contest_links(contest, current_class)
    if contest.is_after_period_voting?
      if contest.visible_range_result.to_s == '1' && contest.visible_range_show.to_s == '1'
        [
          {tag: 'div', content: '[作品一覧(非公開)]', options: {class: "#{current_class} show_result_invisible"}, value: 4.5 },
          {tag: 'div', content: '[投票結果(非公開)]', options: {class: "#{current_class} show_result_invisible"}, value: 4 }
        ]
      elsif contest.visible_range_result.to_s == '1' && contest.visible_range_show.to_s == '0'
        [
      ...
      if contest.is_already_voted?(current_user)
        [
          {tag: 'div', content: '[投票済み]', options: {class: "#{current_class} already_voted"}, value: 3 }
        ]
      elsif contest.visible_range_vote.to_s == '1'
        [
          {tag: 'div', content: '[投票ページ(非公開)]', options: {class: "#{current_class} vote_invisible"}, value: 3 }
        ]
      else
        [
      ...
      if contest.visible_range_vote.to_s == '1'
        [
          {tag: 'div', content: '[投票ページ(非公開)]', options: {class: "#{current_class} vote_invisible"}, value: 2 }
        ]
      else
        [
          {tag: 'div', content: '[投票ページ(準備中)]', options: {class: "#{current_class} vote_invisible"}, value: 2 }
        ]
      end
      ...
      if contest.is_already_submitted?(current_user)
        ...
        [
          {tag: 'div', content: '[作品提出ページ(非公開)]', options: {class: "#{current_class} entry_invisible"}, value: 1 }
        ]
      else
        [
